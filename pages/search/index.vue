...
export default {
  ...
  async asyncData({ store, query }) {
    await store.dispatch('web/product/getProductsData', { query: query.q, page: 1 });  // Start with page 1 by default
  },
  ...
  methods: {
    changePage(page) {
      // Update the URL query with the new page
      this.$router.push({
      ...
      this.$store.dispatch('web/product/getProductsData', { query: this.$route.query.q, page });
    },

    formatPrice(price) {
      // Format price as IDR (Indonesian Rupiah)
